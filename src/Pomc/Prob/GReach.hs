...
encodePush globals sIdGen delta supports q g qState semiconfId_ rightCnxts sccMembers =
  ...
  in do
    ...
    newUnencoded <- newIORef []

    let suppInfo = zip3 suppEnds suppEndsIds suppSemiconfsIds
    suppVarMap  <- foldM (\acc (s, sId, id_) -> do
      encodedContexts <- retrieveEquationsIds (eqMap globals) id_
      let previouslyEncoded = IntSet.intersection rightCnxts encodedContexts
          toBeEncoded = IntSet.filter (`IntSet.notMember` encodedContexts) rightCnxts
          actualrightCnxts
              | IntSet.notMember id_ sccMembers = previouslyEncoded
              | otherwise = rightCnxts
          update rc l
              | IntSet.member rc actualrightCnxts = (sId, (id_,rc)):l
              | otherwise = l
          updatedMap = IntMap.mapWithKey update acc
          cases
            | IntSet.member id_ sccMembers && not (IntSet.null toBeEncoded) = do
              addFixpEqs (eqMap globals) id_ (IntMap.fromSet (const (PushEq [])) toBeEncoded)
              modifyIORef' newUnencoded $ \l -> (QuantVariable (s,g) id_ toBeEncoded):l
              return updatedMap
            | otherwise = return updatedMap
      cases
      ) (IntMap.fromSet (const []) rightCnxts) suppInfo

    let pushVarKeys = [(p, prob_, equations, id_, suppRCs) |
                        (p, id_, prob_, equations) <- newStates,
                        let suppRCs = if IntSet.member id_ sccMembers
                                        then IntSet.fromList suppEndsIds
                                        else IntSet.intersection (IntSet.fromList suppEndsIds) equations
                      ]
        pushVarKeystoEncode = [ QuantVariable (p, newG) id_ toEncodeRCs |
                                (p, _, equations, id_, suppRCs) <- pushVarKeys,
                                IntSet.member id_ sccMembers,
                                let toEncodeRCs =IntSet.filter (`IntSet.notMember` equations) suppRCs,
                                not $ IntSet.null toEncodeRCs
                              ]
        decode (_, prob_, _, pushId, pushRCs)  = [(prob_,pushId, pushRC) | pushRC <- IntSet.toList pushRCs]
        createTerm (rc, suppVarKeys) =
          let term = [(prob_, (pushId,pushRC), suppVarKey) |
                                              (suppSId, suppVarKey) <- suppVarKeys,
                                              (prob_, pushId, pushRC) <- (concatMap decode pushVarKeys),
                                              pushRC == suppSId
                                            ]
            in if null term
              then (rc, PopEq (0,0))
              else (rc, PushEq term)
        createShowTerm (rc, suppVarKeys) = (rc, [(prob_, (pushId,pushRC), suppVarKey) |
                                              (suppSId, suppVarKey) <- suppVarKeys,
                                              (_, prob_, _, pushId, pushRCs) <- pushVarKeys,
                                              pushRC <- IntSet.toList pushRCs,
                                              pushRC == suppSId
                                            ])
        terms = IntMap.fromList . map createTerm . IntMap.toList $ suppVarMap
        showTerms = map createShowTerm . IntMap.toList $ suppVarMap

    forM_ pushVarKeystoEncode $ \(QuantVariable _ id_ toBeEncoded) ->
    ...
    liftSTtoIO $ modifySTRef' (stats globals) $ \s@Stats{equationsCountQuant = acc} -> s{equationsCountQuant = acc + length (terms)}
    --DBG.traceM $ "Encoding push: " ++ show semiconfId_ ++ " = PushEq " ++ show showTerms
    --DBG.traceM ("Return from Encoding push.")
    forM_ pushVarKeystoEncode $ \qv -> encode qv globals sIdGen delta supports sccMembers
    suppVarKeystoEncode <- readIORef newUnencoded
    forM_ suppVarKeystoEncode $ \qv -> encode qv globals sIdGen delta supports sccMembers
...
encodeShift globals sIdGen delta supports _ g qState semiconfId_ rightCnxts sccMembers =
  let qProps = getStateProps (bitenc delta) qState
      shiftEnc (newVars, terms) (p, prob_) = do
        ...
        encodedContexts <- retrieveEquationsIds (eqMap globals) id_
        let previouslyEncoded = IntSet.intersection rightCnxts encodedContexts
            toBeEncoded = IntSet.difference rightCnxts encodedContexts
            actualrightCnxts
              | IntSet.notMember id_ sccMembers = previouslyEncoded
              | otherwise = rightCnxts
            update rc l
              | IntSet.member rc actualrightCnxts = (prob_, (id_,rc)):l
              | otherwise = l
            updatedMap = IntMap.mapWithKey update terms
            cases
              | IntSet.member id_ sccMembers && not (IntSet.null toBeEncoded) = do
                addFixpEqs (eqMap globals) id_ (IntMap.fromSet (const (PushEq [])) toBeEncoded)
                return ((QuantVariable dest id_ toBeEncoded):newVars,
                  updatedMap)
              | otherwise = return (newVars, updatedMap)
        cases
  in do
    --DBG.traceM ("Encoding shift.")
    newStates <- mapM (\(unwrapped, prob_) -> (,prob_) <$> stToIO (wrapState sIdGen unwrapped)) $ (deltaShift delta) qState
    (unencodedVars, terms) <- foldM shiftEnc ([], IntMap.fromSet (const []) rightCnxts) newStates
    let shiftEqs | null terms = error "shift semiconfs should go somewhere!"
                | otherwise = IntMap.map ShiftEq terms
    addFixpEqs (eqMap globals) semiconfId_ shiftEqs
    ...
    --DBG.traceM ("Returning from Shift.")
    forM_ unencodedVars $ \qv -> encode qv globals sIdGen delta supports sccMembers
