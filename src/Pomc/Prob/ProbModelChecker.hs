...
{- |
   Module      : Pomc.ProbModelChecker
   Copyright   : 2023 Francesco Pontiggia
   License     : MIT
   Maintainer  : Francesco Pontiggia
-}

module Pomc.ModelChecker ( Popa(..)
                          , ExplicitPopa(..)
                         ) where
...
data Popa s a = Popa
  ...
  , popaDeltaPop   :: s -> Distr s -- pop transition prob. distribution
...
data ExplicitPopa s a = ExplicitPopa
  ...
  , epopaDeltaPop   ::  [(s, Distr s)]-- pop transition prob. distribution
